...
std::optional<IPv4PortRange> StringToIPv4PortRange(std::string_view subject)
{
  ...
  const auto* read_pos = std::ranges::find_first_of(subject, "-/:");
  ...
  if (*read_pos == '-')
  {
    ...
    const auto* const last_ip_end =
        std::ranges::find_first_of(std::string_view{read_pos, subject.end()}, ":");
  ...
  else if (*read_pos == '/')
  {
    ...
    const auto [parse_end, ec] = std::from_chars(read_pos, subject.end(), network_prefix_length);
  ...
  u16 first_port_u16{};
  {
    ...
    const auto [parse_end, ec] = std::from_chars(read_pos, subject.end(), first_port_u16);
    if (ec != std::errc{})
  ...
  u16 first_last_u16{};
  {
    ...
    const auto [parse_end, ec] = std::from_chars(read_pos, subject.end(), first_last_u16);
